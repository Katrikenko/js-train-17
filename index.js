...
function replaceText(word, replacement, text) {
  ...
  const replacedRegEx = text.replace(regExp, replacement);
...
function checkWord(word, text) {
  ...
  const checkRegExp = regExp.test(text);
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const regexp = RegExp(word, "g");
  const replaceWord = str.replace(regexp, newWord);
  return replaceWord;
}
...
function checkFlags(regex) {
  ...
  const sourceRegExp = regex.source;
  arrFlags.push(sourceRegExp);
...
function findWord(str, word) {
  ...
  const searchWord = str.search(regexp);
  return searchWord;
}
